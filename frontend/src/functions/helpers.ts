...
export const getTimeFromSec = (sec: number, full: boolean = false): string => {
    ...
    if (full)
        return `${h} ${declOfNum(h, lang.declining.hours)} ${m} ${declOfNum(m, lang.declining.minutes)}`
...
export const timeAgo = (sec: number) => {
    if (sec === null || sec <= 0)
        return lang.timeago.recently

    let h = sec/3600 ^ 0
    let m = (sec-h*3600)/60 ^ 0
    let s = sec-h*3600-m*60

    return ((h > 0 ? (h < 10 ? '0' + h : h) + ` ${lang.timeago.h} ` : '')
        + (m > 0 ? (m < 10 ? '0' + m : m) + ` ${lang.timeago.m} ` : '')
        + (s > 0 ? (s < 10 ? '0' + s : s) + ` ${lang.timeago.s}` : '')) + ` ${lang.timeago.ago}`
}
