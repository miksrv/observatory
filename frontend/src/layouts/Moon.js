...
const Moon = (props) => {
    ...
    const MoonPhraseIcon = icons[data.phase_icon]
                ...
                <Grid columns={3}>
                    <Grid.Row stretched>
                        <Grid.Column textAlign='left' width={7}>
                            <div>
                                Возраст (дней): <b>{Number((data.age).toFixed(2))}</b>
                            </div>
                            <div>
                                Освещенность: <b>{Number((data.illumination).toFixed(2)) * 100}%</b>
                            </div>
                            <div>
                                Расстояние (км): <b>{Number((data.distance).toFixed(0))}</b>
                            </div>
                            <div>
                                Фаза Луны: <b>{data.phase_name}</b>
                            </div>
                        </Grid.Column>
                        ...
                        <Grid.Column textAlign='right' width={7}>
                            <div className='icon-info-container'>
                                <WiMoonrise className='icon-info' /> Восход Луны: <b>{moment.unix(data.rise).format("H:mm")}</b>
                            </div>
                            <div className='icon-info-container'>
                                <WiMoonset className='icon-info' /> Закат Луны: <b>{moment.unix(data.set).format("H:mm")}</b>
                            </div>
                            <div>
                                Новолуние: <b>{moment.unix(data.phase_new).format("DD.MM.Y")}</b>
                            </div>
                            <div>
                                Полнолуние: <b>{moment.unix(data.phase_full).format("DD.MM.Y")}</b>
                            </div>
                        </Grid.Column>
                    </Grid.Row>
                </Grid>
